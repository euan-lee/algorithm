...
using namespace std;
/*
void dfs(vector<vector<int>> &game_board,vector<vector<int>> &blocks,vector<int> &visited,int depth){
    ...
    for(int i=0;i<game_board.size();i++){//게임 보드 위치 순회
        for(int j=0;j<game_board[i].size();j++){//게임 보드 위치 순회
            ...
            for(int k=0;k<visited.size();k++){//테이블 사용여부 확인을 위해서
                
                if(game_board[i][j]!=1&&visited[k]!=1){//만약 게임 보드가 비어있고 블럭을 쓸수 있다면~
                    for(int h=0;h<4;h++){//회전한 후  
                        visited[k]=1;
                        //table[k] 회전변환
                        //game_board[i][j]==table[k]
                        dfs(game_board,table,visited,depth++);
                    } 
                }
            }
        }
...
int solution(vector<vector<int>> game_board, vector<vector<int>> table) {
   ...
   for(int i=0;i<blocks.size();i++){
        for(int j=0;j<blocks[i].size();j++){
            printf("(%d %d) ",blocks[i][j].first,blocks[i][j].second);    
        } 
        printf("\n");
    }
